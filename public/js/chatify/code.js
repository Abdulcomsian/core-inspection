  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        // Replace the URL path while keeping the current domain
        let userAvatarUrl = data.user_avatar.replace('http://localhost/storage/users-avatar/', currentDomain + '/images/');
        ...
        img.onerror = function() {
            // Image does not exist or is invalid, set default avatar
            userAvatarUrl = currentDomain + '/build/images/users/avatar-1.jpg';
            setAvatar(userAvatarUrl);
        };
        ...
        function setAvatar(url) {
            $(".messenger-infoView").find(".avatar").css("background-image", 'url("' + url + '")');
            $(".header-avatar").css("background-image", 'url("' + url + '")');
        }
