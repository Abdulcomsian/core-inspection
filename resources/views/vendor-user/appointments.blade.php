            ...
            <div class="card">
                <div class="card-body">
                    <div class="table-responsive table-card">
                        <table class="table table-centered align-middle table-nowrap mb-0" id="vendor-appointments-table">
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            var vendorTable = $('#vendor-appointments-table').DataTable({
                processing: true,
                serverSide: true,
                lengthChange: false,
                searching: true,
                order: [[1, 'desc']],
                ajax: {
                    url: '{{ route('vendor.Appointments') }}',
                    data: function(d) {
                        d.service_category = $('#service-category').val();
                        d.service_type = $('#service-type').val();
                        d.search_input = $('#global-search-input')
                            .val(); // Capture the global search input
                    }
                },
                columns: [{
            ...
            $('#vendor-appointments-table_filter').hide();

            $('#vendor-appointments-table_filter').after(`
    ...
    `);
            ...
            $('#service-category, #service-type, #global-search-input').on('change keyup',
                function() {
                    vendorTable.draw();
                });
        });
