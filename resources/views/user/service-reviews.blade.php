                                ...
                                @foreach ($serviceReviews as $review)
                                    <div class = "row">
                                        <div class = "col-md-6 justify-content-start">
                                            <img src="@if ($review->userDetails->avatar != '') {{ URL::asset('images/' . $review->userDetails->avatar) }}@else{{ asset('assets/images/human-avatar.png') }} @endif"
                                                alt="user-img" class="img-thumbnail rounded-circle"
                                                style = "width:50px;height:50px;body-fit:cover;margin-right:5px;">
                                            <span>{{ $review->userDetails->first_name ?? '' }}
                                                {{ $review->userDetails->last_name }}</span>
                                        </div>
                                        <div class = "col-md-6 ">
                                            <div class=" d-flex justify-content-end">
                                                <ul class="list-unstyled d-flex justify-content-start text-warning mb-0">
                                                    @for ($i = 1; $i < 6; $i++)
                                                        <li><i
                                                                class="{{ $review->ratings >= $i ? 'fas' : 'far' }} fa-star fa-sm"></i>
                                                        </li>
                                                    @endfor
                                                </ul>
                                            </div>
