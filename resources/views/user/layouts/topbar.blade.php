...
if (strpos($avatar, 'http://localhost/storage/users-avatar/') === 0) {
    $avatar = str_replace('http://localhost/storage/users-avatar/', '', $avatar);
}

// Construct the final URL
$avatarUrl = $avatar
    ? URL::asset('images/' . $avatar)
    : URL::asset('build/images/users/avatar-1.jpg');
                            @endphp

                            <img class="rounded-circle header-profile-user" src="{{ $avatarUrl }}"
                                alt="Header Avatar">

                            <span class="text-start ms-xl-2">
                                <span
                                    class="d-none d-xl-inline-block ms-1 fw-medium user-name-text">{{ Auth::user()->last_name }}
                                    {{ Auth::user()->first_name }}</span>
                                @if (Auth::user()->hasRole('admin'))
                                    <span class="d-none d-xl-block ms-1 fs-12 user-name-sub-text">Admin</span>
                                @elseif(Auth::user()->hasRole('vendor_user'))
                                    <span class="d-none d-xl-block ms-1 fs-12 user-name-sub-text">Service
                                        Provider</span>
                                @else
                                    <span class="d-none d-xl-block ms-1 fs-12 user-name-sub-text">System User</span>
                                @endif
                            </span>
                        </span>
                    </button>
...
<div class="modal fade bs-modal-add-balance" tabindex="-1" aria-labelledby="mySmallModalLabel1"
    style="display: none;" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="mySmallModalLabel1">Add Funds</h5>
                ...
                <form action = "{{ route('user.addBalance') }}" method = "POST">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-xxl-12">
                            <div>
                                <label for="basiInput" class="form-label">MTN Number</label>
                        ...
                        <div class="col-xxl-12">
                            <div>
                                ...
                                <input type="number" name="amount" class="form-control" id="mtn-number"
