                    ...
                    <div class="col-xl-12">
                        <div class="card">
                            <div class="card-header align-items-center d-flex">
                                <h4 class="card-title mb-0 flex-grow-1">Approve Service Providers</h4>
                            </div><!-- end card header -->
                            ...
                            <div class="card-body">
                                <div class="table-responsive table-card">
                                    <table class="table table-centered align-middle table-nowrap mb-0">
                                        ...
                                        <tbody>
                                            @isset($unapprovedonBoardings)
                                                @foreach ($unapprovedonBoardings as $index => $unapprovedonBoarding)
                                                    <tr>
                                                        <td>{{ $index + 1 }}</td>
                                                        <td>{{ $unapprovedonBoarding->first_name }}</td>
                                                        <td>{{ $unapprovedonBoarding->last_name }}</td>
                                                        <td>{{ $unapprovedonBoarding->email }}</td>
                                                        <td>Service Provider</td>
                                                        @if ($unapprovedonBoarding->is_approved == 0)
                                                            <td><span class="badge bg-warning text-dark">Pending</span></td>
                                                        @elseif($unapprovedonBoarding->is_approved == 2)
                                                            <td><span class="badge bg-danger text-dark">Rejected</span></td>
                                                        @else
                                                            <td><span class="badge bg-success">Approved</span></td>
                                                        @endif
                                                        @if ($unapprovedonBoarding->profile_approved_status == 0)
                                                            <td><span class="badge bg-warning text-dark">Pending</span></td>
                                                        @elseif($unapprovedonBoarding->profile_approved_status == 2)
                                                            <td><span class="badge bg-danger text-dark">Rejected</span></td>
                                                        @else
                                                            <td><span class="badge bg-success">Approved</span></td>
                                                        @endif
                                                        <td>
                                                            <div class="hstack gap-3 flex-wrap">
                                                                <a href="{{ url('admin/onboarding-vendor-profile', $unapprovedonBoarding->id) }}"
